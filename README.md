...
### 使用 uv（推荐）
```shell
# 创建虚拟环境并安装依赖（需先安装 uv: https://docs.astral.sh/uv/）
uv sync

# 运行脚本（自动使用项目 .venv）
uv run python ./splicing/splicing.py --help
uv run python ./splicing/splicing.py ./examples/safari.MP4 -o out.jpg
```
...
### 系统依赖
需要安装 [ffmpeg](https://ffmpeg.org/)（如 `apt install ffmpeg` / `winget install ffmpeg`）。

## 用法
```sh
# 使用 uv 时
uv run python ./splicing/splicing.py --help
uv run python ./splicing/splicing.py ../examples/a.mp4

# 或激活 .venv 后
python ./splicing/splicing.py --help
python ./splicing/splicing.py ../examples/a.mp4
```
